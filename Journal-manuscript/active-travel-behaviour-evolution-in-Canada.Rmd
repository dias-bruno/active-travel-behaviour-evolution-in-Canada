...
destination_percentual <- gss_episodes_renamed %>%
...
```{r figure-destmodeyearperc, echo=FALSE, out.width="100%", fig.cap="Percentage of walking and cycling trips categorized by destination and year", fig.align="center"}
...
destinations_median <- gss_episodes_renamed %>% 
  group_by(MODE, dest_label, YEAR) %>%
...
destination_boxplots <- gss_episodes_renamed %>% 
...
```{r figure-boxplot, echo=FALSE, out.width="100%", fig.cap="Percentage of walking trips categorized by origin and destination", fig.align="center"}
  ...
  for (mode in unique(gss_heatmap$MODE)) {
    trip_counts <- gss_heatmap %>%
      ...
      destination_order <- c("Home", "Work or school", "Grocery store", 
                             "Others' home", "Health clinic", "Outdoors", "Neighbourhood", 
...
```{r walking-heatmap, echo=FALSE, out.width="100%", fig.cap="Percentage of walking trips categorized by origin and destination", fig.align="center"}
...
```{r cycling-heatmap, echo=FALSE, out.width="100%", fig.cap="Percentage of walking trips categorized by origin and destination", fig.align="center"}
...
selected_functions <- gss_impedances %>% 
  ...
  summarise(Function = first(f_name), 
            "Parameter 1" = first(est_1),
            "Parameter 2" = first(est_2),
            AIC = first(aic),
            .groups = "drop") %>% 
  ...
  across(c("Parameter 1", "Parameter 2"), ~ round(., digits = 2)), 
  across(c(AIC), ~ round(., digits = 0))) %>% 
  rename(Mode = MODE, Year = YEAR, Destination = dest_label)

walking_functions_table <- selected_functions %>%
  filter(Mode == 'Walking') %>% 
  select(Year, Destination, Function, "Parameter 1", "Parameter 2", AIC) %>%
  kable(format = "latex",
        booktabs = TRUE,
        caption = paste0("\\label{walking_functions_table}Impedance functions and AIC for walking trips."), 
        col.names = c("Year", "Destination", "Impedance function","Parameter 1*","Parameter 2*", "AIC")) %>% 
    kable_styling(full_width = FALSE, 
                latex_options=c("scale_down"),
                font_size = 10) %>% 
  row_spec(0, bold = TRUE, align = "c") %>% 
...
biking_functions_table <- selected_functions %>%
  ...
  kable(format = "latex",
        booktabs = TRUE,
        caption = paste0("\\label{biking_functions_table}Impedance functions and AIC for cycling trips."), 
        col.names = c("Year", "Destination", "Impedance function","Parameter 1*","Parameter 2*", "AIC")) %>% 
    kable_styling(full_width = FALSE, 
                latex_options=c("scale_down"),
                font_size = 10) %>% 
...
In total, we fitted `r nrows(selected_functions)` impedance functions. Among the candidate distributions, only the lognormal, gamma, and uniform distributions were selected using our methodology, with the uniform distribution being chosen exclusively for certain cycling destinations. Table \ref{tab:walking_functions_table} displays the selected functions for walking trips, while Table \ref{tab:cycling_functions_table} presents the functions for cycling trips. Appendix A includes the AIC, BIC, and log-likelihood values for all candidate distributions.
