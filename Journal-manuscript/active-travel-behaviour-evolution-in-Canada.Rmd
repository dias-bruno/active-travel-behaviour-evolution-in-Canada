...
```{r active_pop_gender_proportion_table, include = FALSE}
# Active episodes
...
long <- melt(setDT(active_pop_proportion_g[,c("YEAR","GENDER2","cycle_p","walk_p","active_p")]), 
             id.vars = c("YEAR","GENDER2"), variable.name = "Proportion")
```

```{r}
teste <- ggplot(long %>% 
                  filter(Proportion %in% c("walk_p","cycle_p")), aes(x = YEAR, y = value, color = Proportion, linetype = GENDER2)) +
  geom_line(size = 1) +
  geom_point()+
...
```{r active_pop_age_proportion_table, include = FALSE}
# Active episodes
...
long_age <- melt(setDT(active_pop_proportion_age[,c("YEAR","AGE","walk_p","cycle_p","active_p")]), 
             id.vars = c("YEAR","AGE"), variable.name = "Proportion")
```

```{r}
#my_colors <- kelly(length(destination_order))
my_colors_years <- c("#cbd9bf","#bae4bc","#7bccc4","#43a2ca","#0868ac","#053c63","#053c93")

teste <- ggplot(long_age %>% 
                  filter(Proportion %in% c("walk_p","cycle_p")), aes(x = YEAR, y = value, color = AGE)) +
  geom_line(size = 0.8) +
  theme_minimal() + 
  scale_x_continuous(
    breaks = c(1992, 1998, 2005, 2010, 2015, 2022)) + 
  scale_color_manual(values = my_colors_years) +
  facet_wrap(~ as.factor(Proportion))
