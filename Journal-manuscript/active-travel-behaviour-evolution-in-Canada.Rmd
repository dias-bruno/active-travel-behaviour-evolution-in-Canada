...
To facilitate collaboration and further analysis, we updated the `ActiveCA` R Package to include the methodology to obtain impedance functions from the raw data files (GSS surveys). Additionally, we created this paper using literate programming in which the R markdown code to fully reproduce this article is available on our GitHub repository *(include after the review)*, in line with the best practices of spatial data science [@arribas-bel2021; @páez2021]. These contributions improve our understanding of active travel behaviour in Canada and provide a basis for future research and policy-making.
...
When analyzing the two active transportation modes, walking episodes account for `r gss_summary[gss_summary$Mode == 'Walking',]$'Total (%)'`%, while the remaining `r gss_summary[gss_summary$Mode == 'Cycling',]$'Total (%)'`% are cycling episodes. 
The most recent survey (2022) showed that bicycle trips accounted for almost 10% of active travel episodes, reinforcing an increasing trend in the participation since 2005 - the year that marked the lowest representation, as opposed to 1992, when bicycle episodes marked the highest participation in all years (13%). 
...
active_pop_proportion_table <- active_pop_proportion  %>% 
  dplyr::select(YEAR, walk_p, cycle_p, active_p, total) %>%
  kable(format = "latex", booktabs = TRUE, longtable = TRUE, 
        ...
        col.names = c("Year","Walking (%)","Cycling (%)", 
                      "Walking and Cycling (%)", "Total Population")) %>% 
  kable_styling(full_width = FALSE, font_size = 8) %>%
  add_header_above(c(" " = 1, "Active population" = 3, " " = 1))
```

In general, the share of the population with active trip records varied between 7.8% and 14.6% from 1992 to 2022 (Table \ref{tab:active-pop-proportion-table}). The year 1998 recorded the lowest level of active trip participation, with 7.8% (1,892,299 people), while 2010 marked the peak of active participation at 14.5% (4,084,114 people). In 2015, the trend of increasing participation in walking and cycling trips was reversed to the beginning of a decline, confirmed by the 2022 survey. In 2015, around 11% of the population (3,265,846 people) recorded at least one active trip. In 2022, this share of the population fell to 8.9%, the second lowest share in the entire historical series, representing 2,856,564 people out of a total population of 32,136,802 people.

```{r table-active_pop_proportion_table, echo=FALSE, cache=FALSE}
active_pop_proportion_table
```

The decrease in the active population over the years is observed in both genders, Men+ and Women+. However, the decline in the share of the active population was more pronounced among Women+ than Men+, leading to a shift in the pattern where Women+ had historically been the more active gender. Women+ peaked at 14.82% of the population with a record of active episodes in 2010, but dropped to the second-lowest level of 8.84% in 2022, only higher than 1998 when the active representation of 7.66%. In contrast, Men+, who also peaked in 2010 with 15.31% of the population reporting active episodes, declined to 10.05% in 2022, appearing as the gender with higher active population share. When analyzing by transportation mode, in all survey years a higher proportion of Men+ reported at least one cycling episode on the previous day compared to Women+ (ranging from 1.18% to 1.87% for Men+ and 0.18% to 0.66% for Women+). Conversely, Women+ historically reported more walking trips than Men+, but this pattern changed in the most recent survey (2022), reinforcing a trend already present in 2015. In 2022, 8.43% of Women+ reported at least one walking episode, compared to 8.89% of Men+.

```{r active_pop_gender, include = FALSE}
# Active episodes
active_pop_proportion_g <- main_act_eps %>% 
  group_by(PUMFID, WGHT_PER, YEAR, GENDER2) %>%
  summarise(Count = sum(Count)) %>% 
  ...
  summarise(active = round(sum(WGHT_PER[Count > 0], na.rm = TRUE)), 
            ...
            active_p = round((active / total) * 100,2),
            non_active_p = 100 - active_p) %>%
  dplyr::select(YEAR, GENDER2, active, active_p, non_active, non_active_p, total)

# walking episodes
walk_pop_proportion_g <- main_act_eps %>% 
  filter(MODE == 'Walking') %>% 
  group_by(PUMFID, WGHT_PER, YEAR, GENDER2) %>%
  summarise(Count = sum(Count)) %>% 
  group_by(YEAR, GENDER2) %>%
  summarise(walk = round(sum(WGHT_PER[Count > 0], na.rm = TRUE)), 
            non_walk = sum(WGHT_PER[Count < 1], na.rm = TRUE)) %>%
  left_join(active_pop_proportion_g[,c('YEAR','GENDER2','total')], by = c('YEAR', 'GENDER2')) %>% 
  mutate(walk_p = round((walk/total) * 100,2),
         non_walk_p = 100 - walk_p) %>%
  dplyr::select(YEAR, GENDER2, walk, walk_p, non_walk, non_walk_p)

# Cycling episodes
cycle_pop_proportion_g <- main_act_eps %>% 
  filter(MODE == 'Cycling') %>% 
  group_by(PUMFID, WGHT_PER, YEAR, GENDER2) %>%
  summarise(Count = sum(Count)) %>% 
  group_by(YEAR, GENDER2) %>%
  ...
  mutate(cycle_p = round((cycle/total) * 100,2),
         non_cycle_p = 100 - cycle_p) %>%
  dplyr::select(YEAR, GENDER2, cycle, cycle_p, non_cycle, non_cycle_p)

# Joining tables
active_pop_proportion_g <- active_pop_proportion_g %>%
  left_join(walk_pop_proportion_g, by = c('YEAR', 'GENDER2')) %>%
  left_join(cycle_pop_proportion_g, by = c('YEAR', 'GENDER2')) %>%
  dplyr::select(YEAR, GENDER2, walk, walk_p, non_walk, non_walk_p,
                cycle, cycle_p, non_cycle, non_cycle_p,
                active, active_p, non_active, non_active_p, total)

library(data.table)
long <- melt(setDT(active_pop_proportion_g[,c("YEAR","GENDER2","cycle_p","walk_p","active_p")]), 
             id.vars = c("YEAR","GENDER2"), variable.name = "Proportion") %>% mutate(Proportion = case_when(
                 Proportion == 'walk_p' ~ "Walking", 
                 Proportion == 'cycle_p' ~ "Cycling", 
                 Proportion == 'active_p' ~ "Active"),
...
active_pop_gender_graph <- ggplot(long %>% 
                  filter(Mode %in% c("Walking","Cycling")), aes(x = Year, y = value, color = Mode, linetype = Gender)) +
  geom_line(linewidth = 1) +
  geom_point()+
  geom_label(aes(label = round(value, 2)), size = 3, vjust = 0) +
  theme_minimal() +
    labs(y = "Percentage (%)") +  
  scale_x_continuous(
    breaks = c(1992, 1998, 2005, 2010, 2015, 2022))
...
```{r gender-prevalence, echo=FALSE, out.width="100%", fig.cap="Population with active trip episodes by transportation mode and gender."}

knitr::include_graphics(paste0(here::here(),"/Journal-manuscript/figures/active_pop_gender_graph.jpg"))
```
When we analyze the population by age group, the youngest (those between 15 and 24 years) stand out as the most active group, ranging from 9.3% to 26.88%, and marking 21.47% in the most recent survey. This is the only group that did not show a decrease in the prevalence of active participation over the years, increasing from a low of 17.91% in 2015 to 21.47% in 2022 - although still below the levels recorded in 2010 (24.01%) and 2005 (26.88%). Historically, prevalence decreases as age increases. However, in the most recent survey (2022), as well as in 2005, the oldest group (75 years and older) presented the third-highest prevalence (7.02%), surpassing the groups aged 35 to 44 years (5.78%), 45 to 54 years (6.30%), 55 to 64 years (6.24%), and 65 to 74 years (6.96%).

The analysis by mode shows a similar trend. However, for cycling, the second youngest group (aged 25 to 34 years) had the highest prevalence in the 2022 survey (1.62%), surpassing the youngest group (1.58%). For all other age groups, cycling prevalence decreases as age increases, approaching 0% for the oldest group (75 years and older).

```{r active_pop_age_proportion, include = FALSE}
# Active episodes
active_pop_proportion_age <- main_act_eps %>% 
  group_by(PUMFID, WGHT_PER, YEAR, AGE) %>%
  summarise(Count = sum(Count)) %>% 
  group_by(YEAR, AGE) %>%
  summarise(active = round(sum(WGHT_PER[Count > 0], na.rm = TRUE)), 
            non_active = round(sum(WGHT_PER[Count < 1], na.rm = TRUE)),
            ...
            active_p = round((active / total) * 100,2),
            non_active_p = 100 - active_p) %>%
  dplyr::select(YEAR, AGE, active, active_p, non_active, non_active_p, total)

# walking episodes
walk_pop_proportion_age <- main_act_eps %>% 
  filter(MODE == 'Walking') %>% 
  group_by(PUMFID, WGHT_PER, YEAR, AGE) %>%
  summarise(Count = sum(Count)) %>% 
  group_by(YEAR, AGE) %>%
  summarise(walk = round(sum(WGHT_PER[Count > 0], na.rm = TRUE)), 
            non_walk = sum(WGHT_PER[Count < 1], na.rm = TRUE)) %>%
  left_join(active_pop_proportion_age[,c('YEAR','AGE','total')], by = c('YEAR', 'AGE')) %>% 
  mutate(walk_p = round((walk/total) * 100,2),
         non_walk_p = 100 - walk_p) %>%
  dplyr::select(YEAR, AGE, walk, walk_p, non_walk, non_walk_p)

# Cycling episodes
cycle_pop_proportion_age <- main_act_eps %>% 
  filter(MODE == 'Cycling') %>% 
  group_by(PUMFID, WGHT_PER, YEAR, AGE) %>%
  summarise(Count = sum(Count)) %>% 
  group_by(YEAR, AGE) %>%
  ...
  mutate(cycle_p = round((cycle/total) * 100,2),
         non_cycle_p = 100 - cycle_p) %>%
  dplyr::select(YEAR, AGE, cycle, cycle_p, non_cycle, non_cycle_p)

# Joining tables
active_pop_proportion_age <- active_pop_proportion_age %>%
  left_join(walk_pop_proportion_age, by = c('YEAR', 'AGE')) %>%
  left_join(cycle_pop_proportion_age, by = c('YEAR', 'AGE')) %>%
  dplyr::select(YEAR, AGE, walk, walk_p, non_walk, non_walk_p,
                cycle, cycle_p, non_cycle, non_cycle_p,
                active, active_p, non_active, non_active_p, total)

# Creating a long table
long_age <- melt(setDT(active_pop_proportion_age[,c("YEAR","AGE","walk_p","cycle_p","active_p")]), 
             id.vars = c("YEAR","AGE"), variable.name = "Proportion") %>% mutate(Proportion = case_when(
                 Proportion == 'walk_p' ~ "Walking", 
                 Proportion == 'cycle_p' ~ "Cycling", 
                 Proportion == 'active_p' ~ "Active"),
                 Proportion = as.factor(Proportion)) %>% 
...
#Age Graph
active_pop_age_proportion <- ggplot(long_age %>% 
                  filter(Mode %in% c("Active")), 
                aes(x = Year, y = value, color = AGE)) +
  geom_line(size = 0.75) +
  theme_minimal() +
  labs(y = "Percentage (%)") +  
  geom_label(aes(label = round(value, 2)), size = 3, vjust = 0) +
  scale_x_continuous(breaks = c(1992, 1998, 2005, 2010, 2015, 2022)) +
  scale_color_brewer(palette = "Spectral") + 
  facet_wrap(~ as.factor(Mode)) +
  labs(color = "Age")

# Save the combined plot to a file
ggsave(file = paste0(here::here(),"/Journal-manuscript/figures/active_pop_age_graph_active.jpg"), 
       plot = active_pop_age_proportion,
       width = 20, 
       height = 10, 
       units = "cm", 
       dpi = 300)
...
```{r age-prevalence, echo=FALSE, out.width="100%", fig.cap="Population with active trip episodes by transportation mode and age group."}
...
The number of people with active trip episodes is mainly influenced by walking episodes. On average, over 90% of the recorded active trips involve individuals with walking episodes. Analyzing the number of episodes per person for the general population, the average value for each year is approximately 0.22, ranging from a minimum of 0.15 in 1998 to a maximum of 0.30 in 2010. Considering only the population with active episodes recorded, the average is around 2 active episodes per person, varying from a maximum of 2.11 episodes in 2005 to a minimum of 1.77 episodes per person in 1992. The average of 2 episodes for the active population is expected, as it reflects the two trips typically required to travel to and from a specific location. Figure \ref{fig:eps-per-person-fig} presents these values broken down by mode, walking and cycling. 

```{r include = FALSE}
# total episodes
resume_eps_by_type <- gss_episodes_renamed %>%
     group_by(YEAR, MODE) %>%
     dplyr::summarise(
         Episodes = sum(WGHT_EPI),
         .groups = "drop") %>%
  rename(Mode = MODE)

resume_eps <- resume_eps_by_type %>%
  group_by(YEAR) %>% 
  dplyr::summarise(Episodes = sum(Episodes),
         .groups = "drop") %>%  
    mutate(Mode = 'Active_eps') %>%
  select(YEAR, Mode, Episodes)

# total pop
pop <- active_pop_proportion %>% 
  dplyr::select(YEAR, walk, cycle, active, total) %>% 
  rename(Walking_pop = walk, 
         Cycling_pop = cycle, 
         Active_pop = active, 
         Total_pop = total)

ep_per_person <- resume_eps %>% 
  left_join(pop, by = "YEAR") %>% 
  mutate(eps_per_total_pop = Episodes/Total_pop, 
         eps_per_active_pop = Episodes/Active_pop)

# Per type of transportation 

tab_total <- resume_eps_by_type %>% 
  left_join(ep_per_person %>% dplyr::select(YEAR, Total_pop), by = 'YEAR') %>% 
  mutate(type = 'Total') %>% 
  rename(Pop = Total_pop)

tab_active <- resume_eps_by_type %>% 
  left_join(ep_per_person %>% dplyr::select(YEAR, Active_pop), by = 'YEAR') %>% 
  mutate(type = 'Active') %>% 
  rename(Pop = Active_pop) 

tab_walk <- resume_eps_by_type %>% 
  left_join(ep_per_person %>% dplyr::select(YEAR, Walking_pop), by = 'YEAR') %>% 
  mutate(type = 'Walking') %>% 
  rename(Pop = Walking_pop) %>% 
  filter(Mode == 'Walking')
  

tab_bike <- resume_eps_by_type %>% 
  left_join(ep_per_person %>% dplyr::select(YEAR, Cycling_pop), by = 'YEAR') %>% 
  mutate(type = 'Cycling') %>% 
  rename(Pop = Cycling_pop) %>% 
  filter(Mode == 'Cycling')

combined_data <- bind_rows(tab_total,tab_active,tab_walk,tab_bike) %>%
  mutate(ep_per_person = Episodes/Pop)
```

```{r eps-per-person-act-total, include = FALSE}
# Episodes per person Figure (active and total pop)
eps_per_person_fig <- ggplot(combined_data %>% 
                               filter(type %in% c('Active', 'Total')), 
                             aes(x = YEAR, y = ep_per_person, color = Mode)) +
  geom_line(size = 1) +
  geom_point()+
  ...
  labs(
    x = "Year",
    y = "Episodes",
    color = "Mode") + facet_wrap(~ as.factor(type))

# Saving figure
ggsave(file = paste0(here::here(),"/Journal-manuscript/figures/episodes_per_person_act_total.jpg"),
...
```{r eps-per-person-act-total-fig, echo=FALSE, out.width="100%", fig.cap=paste0("Number of active episodes by person."), fig.align="center"}

knitr::include_graphics(paste0(here::here(),"/Journal-manuscript/figures/episodes_per_person_act_total.jpg"))
```


When we split the walking and cycling episodes by the active population and the total population (Figure \ref{fig:eps-per-person-act-total-fig}), we observe that each active person recorded between 1.6 and 1.97 walking episodes, with the minimum in 1992 and the maximum in 2005. This number dropped to 1.87 in 2015, followed by a slight recovery to 1.91 in 2022. For cycling episodes, considering only the active population, there has been an increasing trend since 1998, rising from 0.14 to 0.20 cycling episodes per active person in 2022. However, when we consider the total population, cycling episodes remained stable at 0.02 episodes per person. Walking episodes per total population showed the lowest value (0.17) since 1998, when the minimum of 0.14 episodes per person was recorded.

```{r eps-per-person-walk-bike, include = FALSE}
# Episodes per person Figure (active and total pop)
eps_per_person_fig <- ggplot(combined_data %>% 
                               filter(type %in% c('Walking', 'Cycling')), 
                             aes(x = YEAR, y = ep_per_person, color = Mode)) +
  geom_line(size = 1) +
  geom_point()+
  geom_label(aes(label = round(ep_per_person, 2)), size = 3, vjust = 0) +
...
# Saving figure
ggsave(file = paste0(here::here(),"/Journal-manuscript/figures/episodes_per_person_by_mode.jpg"),
       plot = eps_per_person_fig,
       width = 16, 
       height = 8, 
       units = "cm", 
...
An interesting pattern emerges when we analyze the episodes by type of active population (Figure \ref{fig:eps-per-person-by-mode-fig}). When we divide the number of walking episodes by the population with recorded walking trips (pedestrians), and the number of cycling episodes by the population with recorded cycling trips (cyclists), we observe an increasing trend for both rates. The results shown in Figures \ref{fig:eps-per-person-act-total-fig} and \ref{fig:eps-per-person-by-mode-fig}, along with the data on the percentage of respondents with active trips, indicate that: first, the number of people with active episodes has been decreasing over time; however, among those with recorded active trips, the number of episodes per person is increasing for both transportation modes, walking and cycling.

```{r eps-per-person-by-mode-fig, echo=FALSE, out.width="100%", fig.cap=paste0("Number of active episodes by person, considering the walking population and cyclists."), fig.align="center"}

knitr::include_graphics(paste0(here::here(),"/Journal-manuscript/figures/episodes_per_person_by_mode.jpg"))
```
