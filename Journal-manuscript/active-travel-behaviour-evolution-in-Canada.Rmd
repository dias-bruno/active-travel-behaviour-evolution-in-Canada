...
person_table <- main_act_eps %>%
  ...
  summarise(
    ...
    active_eps = walk_eps + cycle_eps,
    has_walk = walk_eps > 0,
    has_cycle = cycle_eps > 0,
    has_active = active_eps > 0,
    .groups = "drop"
  )

eps_per_person_table <- person_summary %>%
  group_by(YEAR) %>%
  summarise(
    total_walk_eps = sum(walk_eps),
    total_cycle_eps = sum(cycle_eps),
    total_active_eps = sum(active_eps),
    n_walkers = sum(WGHT_PER[has_walk]),
    n_cyclists = sum(WGHT_PER[has_cycle]),
    n_active = sum(WGHT_PER[has_active]),
    walk_per_walker = total_walk_eps / n_walkers,
    cycle_per_cyclist = total_cycle_eps / n_cyclists,
    active_per_active = total_active_eps / n_active,
    .groups = "drop"
  ) %>%
  select(YEAR, walk_per_walker, cycle_per_cyclist, active_per_active)

# Evaluate if I will include this or not 
```
